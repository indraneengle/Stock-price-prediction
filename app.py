...
def index():
    ...
    # next collect inputs from html page
...
def predict():
    ...
    selling_date = date(sell_year, sell_month, sell_day)  # end date
    print(selling_date)

    buy_year = int(start_date.split('-')[0])    # check if works with srt/int
    ...
    for i in df.index:
        ...
        # print(type(sell_date_price))
        # print(type(i.date()))
        # print(type(selling_date))
    ...
    open_model.fit(open_x_train, open_y_train, epochs=10)
    ...
    # result = model.predict([[start_date_price,year,month,day]])
    result=Profit_loss
